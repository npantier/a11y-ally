  ...
  return (
    <div className="aa-report">
      <p className="aa-summary">
        <strong>{summary.passes} passing</strong> · {summary.critical} critical ·{' '}
        {summary.serious} serious · {summary.moderate} moderate · {summary.minor} minor
      </p>
      {ORDER.map((severity) => {
...
function FindingRow({ finding, onSelect }: { finding: Finding; onSelect: (s: string) => void }) {
  return (
    <details className="aa-finding">
      ...
      <a href={finding.helpUrl} target="_blank" rel="noreferrer">How to fix</a>
      <ul>
        {finding.nodes.map((n, i) => (
          <li key={i}>
            <button type="button" onClick={() => onSelect(n.selector)}>{n.selector}</button>
            <code>{n.html}</code>
          </li>
        ))}
      </ul>
